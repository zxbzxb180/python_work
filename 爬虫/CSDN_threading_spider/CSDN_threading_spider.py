...
from 爬虫.CSDN_spider.models import *
...
class ParseAuthorBlogThread(Thread):
    def run(self):
        while 1:
            try:
                url = author_blog_list.pop()
            except IndexError as e:
                time.sleep(1)
                continue
            print('开始获取作者博客页面：{}'.format(url))
...
class ParseAuthorThread(Thread):
    def run(self):
        while 1:
            ...
            author_blog_list.append(blog_url)
...
class ParseTopicListThread(Thread):
    def run(self):
        while 1:
            ...
            for tr in all_trs:
                has_topic = tr.xpath(".//td[1]/span/text()").extract()
                if not has_topic:
                    continue
                status = has_topic[0]
                score = tr.xpath(".//td[2]/em/text()").extract()[0]
                topic_url_all = tr.xpath(".//td[3]/a[contains(@class, 'forums_title')]/@href").extract()
                if len(topic_url_all) != 1:
                    topic_url = parse.urljoin(domain,topic_url_all[-1])
                else:
                    topic_url = parse.urljoin(domain, topic_url_all[0])
            ...
            # if next_page:
            #     next_url = parse.urljoin(domain, next_page[0])
            #     topic_list_url.append(next_url)

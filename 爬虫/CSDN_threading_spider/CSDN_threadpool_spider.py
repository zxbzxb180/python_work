...
def get_cookie():
    ...
    print('获取到cookie：{}'.format(cookie_dict))
    return cookie_dict


def parse_topic(url):
    # 获取帖子详情及回复
    print('解析帖子详情页面：{}'.format(url))
...
def parse_author(url):
    ...
    executor.submit(parse_author_blog, blog_url)


def parse_list(url):
    # 获取帖子列表页
    print('解析帖子列表页：{}'.format(url))
    res_text = requests.get(url, cookies=cookie_dict, headers=headers).text
    sel = Selector(text=res_text)
...
if __name__ == '__main__':
    ...
    for last_url in last_url_list:
        executor.submit(parse_list, last_url)
